...
static int dsi_display_res_init(struct dsi_display *display)
{
	...
	TP_Panel =  display->panel;
	DSI_ERR("[%s]drm: change display->panel to TP_Panel, TP_Panel->name = %s\n", __func__, TP_Panel->name);
	...
	DSI_INFO("INIT_DELAYED_WORK: dsi_display_gamma_read_work\n");
		...
		if (cur_mode->pixel_clk_khz != adj_mode->pixel_clk_khz) {
			if (dyn_clk_caps->dyn_clk_support) {
				...
				if ((adj_mode->dsi_mode_flags &
					DSI_MODE_FLAG_VRR) &&
					(!dyn_clk_caps->maintain_const_fps)) {
					DSI_INFO("dfps and dyn clk not supported in same commit\n");
					rc = -ENOTSUPP;
					goto error;
				}
...
int dsi_display_get_serial_number(struct drm_connector *connector)
{
	...
	if (count) {
		...
		//DSI_INFO("This panel not support serial number.\n");
	}
