...
#define SOC_DATA_REG_0                0xB1A0
#define SOC_FLAG_REG                  0xB1A0
...
#ifdef DEBUG_SKIN_USB_THERM
static int __debug_skin_temp;
module_param_named(
	debug_skin_temp, __debug_skin_temp, int, 0644
);
static int __debug_usb_temp;
module_param_named(
	debug_usb_temp, __debug_usb_temp, int, 0644
);
#endif
...
#define PD_PANELON_CURRENT_UA		3000000  /* @BSP modify PD from 2A to 3A */
#define PD_PANELOFF_CURRENT_UA		3000000  /* @BSP modify PD from 2A to 3A */
				...
				const union power_supply_propval *val)
{
	...
	if (chg->pd_active) {
		...
		/* @bsp, 2020/07/03 try switching fast charge */
		if (!chg->dash_present &&
...
#define PD_SKIN_THERMAL_NORMAL_CURRENT_UA    3000000  /* @BSP modify PD from 2A to 3A */
...
static void pd_skin_thermal_check_work(struct work_struct *work)
{
	...
	// Measure the current skin thermal temperature
	op_get_skin_thermal_temp(chg);
	is_skin_temp_high = is_skin_thermal_level(SKIN_THERM_LEVEL_HIGH);
	is_skin_temp_medium = is_skin_thermal_level(SKIN_THERM_LEVEL_MEDIUM);
	if (is_call_on)
...
/* @BSP limit battery current fcc */
static int default_bat_temp_flag;
...
static int get_prop_batt_temp(struct smb_charger *chg)
{
	...
	if (rc) {
		...
		/* @BSP limit battery current fcc */
		default_bat_temp_flag = 1;
	} else {
		default_bat_temp_flag = 0;
	}
...
static void handle_dash_skin_therm_temp(struct smb_charger *chg, int batt_temp)
{
	...
	if (!chg->oem_lcd_is_on) { // LCD Off
		...
		/* @BSP change log */
		smblib_dbg(chg, PR_FAST_DEBUG, "batt: current=%d,temp=%d\n", batt_curr_ma, batt_temp);
		...
		if (!chg->is_skin_thermal_medium) {
			if (!chg->is_skin_thermal_high &&
			     ((chg->skin_thermal_temp >=
			       ...
			       batt_temp >= 393 ||
			       (batt_temp >= 390 && batt_curr_ma < -4900))) { // >=39  /* @BSP OEM modify */
				chg->is_skin_thermal_medium = true;
			}
		} else {
			if (chg->skin_thermal_temp <
...
/* @BSP define same in fastchg_dash */
...
static void handle_dash_battery_therm_temp(struct smb_charger *chg, int batt_temp)
{
	if (batt_temp > 435) {  // > 43.5 2A
		chg->is_battery_temp_limit = CURRENT_LIMIT_2;
	} else if ((batt_temp > 425) && (batt_temp <= 435)) {  //42.5 -- 43.5
		if ((chg->is_battery_temp_limit == 2) && (batt_temp >= 430))
			chg->is_battery_temp_limit = CURRENT_LIMIT_2;//2A
		else
			chg->is_battery_temp_limit = CURRENT_LIMIT_3;//3A
	} else if ((batt_temp > 410) && (batt_temp <= 425)) {  //41--42.5
		if (chg->is_battery_temp_limit <= CURRENT_LIMIT_3)
			chg->is_battery_temp_limit = CURRENT_LIMIT_3;//3A
		else
			chg->is_battery_temp_limit = CURRENT_LIMIT_4;//4A
	} else if ((batt_temp > 390) && (batt_temp <= 410)) {  //39--41
		if (chg->is_battery_temp_limit <= CURRENT_LIMIT_4)
			chg->is_battery_temp_limit = CURRENT_LIMIT_5;//5A
		else
			chg->is_battery_temp_limit = CURRENT_LIMIT_6;//6A
	} else if ((batt_temp > 160) && (batt_temp <= 390)) { //16--39
		chg->is_battery_temp_limit = CURRENT_LIMIT_6;//6A
	} else if ((batt_temp > -20) && (batt_temp <= 160)) { //-2--16
		chg->is_battery_temp_limit = CURRENT_LIMIT_4;//4A
	} else {
		chg->is_battery_temp_limit = CURRENT_LIMIT_2;//2A
	}
...
static int op_get_skin_thermal_temp(struct smb_charger *chg)
{
...
#ifdef DEBUG_SKIN_USB_THERM
	if (*chg->debug_skin_temp > 0) {
		pr_info("normal skin=(%d),fake skin(%d)\n",
			chg->skin_thermal_temp,
			*chg->debug_skin_temp);
		chg->skin_thermal_temp = *chg->debug_skin_temp;
	}
#endif
	...
	if (chg->pd_active) { // PD charging skin therm
		...
		/* @BSP add dash battery_temp check */
		handle_dash_battery_therm_temp(chg, batt_temp);
	}
...
bool is_skin_thermal_level(int check_level)
{
	...
	switch (check_level) {
	case SKIN_THERM_LEVEL_HIGH:
		is_level_set = g_chg->is_skin_thermal_high;
		break;

	case SKIN_THERM_LEVEL_LITTLE_HIGH:
		is_level_set = g_chg->is_skin_thermal_little_high;
		break;

	case SKIN_THERM_LEVEL_MEDIUM:
		is_level_set = g_chg->is_skin_thermal_medium;
		break;
...
/* @BSP add dash battery_temp check */
int is_battery_temp_level(void)
{
	return g_chg->is_battery_temp_limit;
}

// Interface to measure skin thermal temperature in Oneplus dash charger driver
void check_skin_thermal_temp(void)
{
	op_get_skin_thermal_temp(g_chg);
}
...
bool check_call_on_status(void)
{
	...
	pr_info("is_call_on=(%d), is_video_call_on=(%d)\n",
		is_call_on, is_video_call_on);

	if (g_chg->pd_active)
		return is_call_on;

	return (is_call_on || is_video_call_on);
}
...
static int get_usb_temp(struct smb_charger *chg)
{
	...
	smblib_dbg(chg, PR_FAST_DEBUG, "connecter,Ibatt:%d,batt_temp:%d\n",
				get_prop_batt_current_now(chg) / 1000,
...
#ifdef DEBUG_SKIN_USB_THERM
	if (*chg->debug_usb_temp > 0) {
		pr_info("normal usb_therm=(%d),fake usb_therm(%d)\n",
			temp, *chg->debug_usb_temp);
		temp = *chg->debug_usb_temp;
	}
#endif
...
static void smblib_panel_register_work(struct work_struct *work)
{
	...
	} else if (tp_active_panel) { /* @bsp for tp notify */
		rc = drm_panel_notifier_register(tp_active_panel, &chg->msm_drm_notifier);
		if (rc)
			pr_err("Smb unable to register TP notifier: %d\n", rc);
		else
			pr_err("register notifier TP drm panel success!");
	} else {
...
/* @bsp, 2019/07/05 Battery & Charging porting */
	...
	/* @BSP add dash battery_temp check */
	chg->is_battery_temp_limit = CURRENT_LIMIT_6;

#ifdef DEBUG_SKIN_USB_THERM
	chg->debug_skin_temp = &__debug_skin_temp;
	chg->debug_usb_temp = &__debug_usb_temp;
#endif
