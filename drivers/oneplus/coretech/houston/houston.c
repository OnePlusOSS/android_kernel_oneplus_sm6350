...
static unsigned int cpufreq_hispeed[HT_CLUSTERS] = { 1209600, 1612800};
		...
		unsigned long *util, unsigned long *hi_util)
{
	...
	case 6: case 7: hus = &ht_utils[1]; break;
	default:
		/* should not happen */
...
static void ht_collect_system_data(struct ai_parcel *p)
{
	...
	p->utils[7] = ht_utils[1].utils[1]? (u64) *(ht_utils[1].utils[1]): 0;
...
static void ht_collect_data(void)
{
	...
	monitor.buf->data[idx][HT_UTIL_7] = ht_utils[1].utils[1]? (u64) *(ht_utils[1].utils[1]): 0;
