...
static fw_check_state goodix_fw_check(void *chip_data, struct resolution_info *resolution_info, struct panel_info *panel_data)
{
	...
	do {
		ret = goodix_i2c_read_dbl_check(chip_info->client, chip_info->reg_info.GTP_REG_FW_CHK_MAINSYS, reg_val, 1);

		if (ret < 0) {
			TPD_INFO("%s,check i2c error!\n", __func__);
			return ret;
		}
		if (reg_val[0] != 0xFC) {
...
static u32 goodix_u32_trigger_reason(void *chip_data, int gesture_enable, int is_suspended)
{
	...
	if (unlikely((chip_info->touch_data[0] & GOODIX_FINGER_IDLE_EVENT) == GOODIX_FINGER_IDLE_EVENT)) {
		goto IGNORE_CLEAR_IRQ;
	}
...
//	}
	...
	switch(mode) {
		...
		case MODE_SLEEP:
			...
			//}
			//break
			ret = goodix_power_control(chip_info, false);
                        if (ret < 0) {
		...
		case MODE_TOUCH_HOLD:
			ret = goodix_enable_fingerprint_touchhold(chip_info, flag);
			if (ret < 0) {
				TPD_INFO("%s: enable touchhold mode : %d failed\n", __func__, flag);
			}
			break;

		case MODE_GESTURE_SWITCH:
			ret = goodix_enable_gesture_mask(chip_info, flag);
			if (ret < 0) {
...
static int goodix_enable_gesture_mask(void *chip_data, uint32_t enable)
{
	...
	TPD_INFO("%s, enable = %d\n", __func__, enable);
	if (enable) {
		ret = goodix_send_cmd(chip_info, GTP_CMD_GESTURE_MASK, 0);/*mask all gestures */
	} else {
...
static void goodix_screenon_fingerprint_info(void *chip_data, struct fp_underscreen_info *fp_tpinfo)
{
	...
	TPD_INFO("%s: Entered into fingerprint function *****************\n", __func__);
...
struct touchpanel_operations goodix_ops = {
		...
		struct gtx8_ts_test *ts_test)
{
	...
	if(ts_test->error_count) {
		snprintf(file_data_buf, 128, "/sdcard/TpTestReport/screenOn/tp_testlimit_%02d%02d%02d-%02d%02d%02d-fail-utc.csv",
				(rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
				rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
	} else {
		snprintf(file_data_buf, 128, "/sdcard/TpTestReport/screenOn/tp_testlimit_%02d%02d%02d-%02d%02d%02d-pass-utc.csv",
				(rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
				rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
	...
	filp = filp_open(file_data_buf, O_RDWR | O_CREAT, 0);
	if (IS_ERR(filp)) {
...
static int goodix_tp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	TPD_INFO("%s: Entered into goodix_tp_probe got called *****************\n", __func__);
...
static int __init tp_driver_init(void)
{
	...
	TPD_INFO("%s: Entered into tp_driver_init  *****************\n", __func__);
...
{
	TPD_INFO("%s: Entered into tp_driver_exit *****************\n", __func__);
