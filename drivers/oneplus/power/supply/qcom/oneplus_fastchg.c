...
/* BSP modify for MMI data*/
#include <linux/hardware_info.h>
#define BATTERY_TYPE		"OP_ATL_4P45_4300mAH"
...
struct fastchg_device_info {
	...
	bool is_call_on;
	bool is_lcd_on;
	int is_battery_temp_limit; /* @BSP add dash battery_temp check */
...
/* @BSP add dash battery_temp check. begin */
	...
	if (di->is_call_on) { // Normal call or Video call on
		...
		if (di->is_lcd_on) {
			if (di->is_skin_thermal_high) {
				...
				if (di->is_battery_temp_limit < CURRENT_LIMIT_3) {
					dash_write_4bits(di, di->is_battery_temp_limit);
					pr_err("LITTLE HIGH:dash command sent %d for battery_temp\n",
						di->is_battery_temp_limit);
				} else {
				...
				if (di->is_battery_temp_limit < CURRENT_LIMIT_4) {
					dash_write_4bits(di, di->is_battery_temp_limit);
					pr_err("MEDIUM:dash command sent %d for battery_temp\n",
						di->is_battery_temp_limit);
				} else {
				...
				if (di->is_battery_temp_limit < CURRENT_LIMIT_5) {
					dash_write_4bits(di, di->is_battery_temp_limit);
					pr_err("NORMAL:dash command sent %d for battery_temp\n",
						di->is_battery_temp_limit);
				} else {
			...
			if (di->is_skin_thermal_high) {
				...
				if (di->is_battery_temp_limit < CURRENT_LIMIT_6) {
					dash_write_4bits(di, di->is_battery_temp_limit);
					pr_err("NORMAL:dash command sent %d for battery_temp\n",
						di->is_battery_temp_limit);
				} else {
...
/* @BSP add dash battery_temp check. end */
	...
	int remain_cap = 0;
		...
		case DASH_NOTIFY_ALLOW_READING_IIC:
			if (arg == DASH_NOTIFY_ALLOW_READING_IIC + 1) {
				...
				check_skin_thermal_temp();
				di->is_skin_thermal_high = is_skin_thermal_level(SKIN_THERM_LEVEL_HIGH);
				di->is_skin_thermal_little_high = is_skin_thermal_level(SKIN_THERM_LEVEL_LITTLE_HIGH);
				di->is_skin_thermal_medium = is_skin_thermal_level(SKIN_THERM_LEVEL_MEDIUM);
				/* @BSP add dash battery_temp check */
				di->is_battery_temp_limit = is_battery_temp_level();
				di->is_call_on = check_call_on_status();
				di->is_lcd_on = check_lcd_on_status();
				handle_dash_charge_current(di);
			}
			break;
