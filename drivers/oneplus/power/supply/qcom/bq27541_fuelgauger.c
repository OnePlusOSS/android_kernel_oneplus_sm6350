...
/* @BSP modify work delay timer */
#define BQ27541_INIT_DELAY   50//((HZ)*1)
		...
		if (ret.intval == POWER_SUPPLY_STATUS_FULL) {
			...
			if (soc - di->soc_pre > 0) {
				...
				if ((bq27541_di->fastchg_started
					&& time_last < 10)  /* @BSP change 20S to 10S */
				|| (!bq27541_di->fastchg_started
				&& time_last < 30))
					return di->soc_pre;
				di->saltate_counter = 0;
...
static int bq27541_get_battery_soc(void)
{
	...
	unsigned long soc_enter_time;

	get_current_time(&soc_enter_time);
	if ((bq27541_di->soc_pre == DEFAULT_INVALID_SOC_PRE)
		|| bq27541_di->allow_reading
		|| ((soc_enter_time - bq27541_di->soc_last_time > 1)
		&& flag_read)) {
		flag_read = false;
		soc = bq27541_battery_soc(bq27541_di, 0);
		get_current_time(&bq27541_di->soc_last_time);
		flag_read = true;
	} else {
		soc = bq27541_di->soc_pre;
	}
	return soc;
}
