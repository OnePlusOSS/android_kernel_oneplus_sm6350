...
void translated_command_converter(char cmd, struct etspi_data *egistec)
{
	...
	case NAVI_EVENT_OFF: /* finger up */
		if (g_KeyEventRaised == false) {
			...
			if (g_SingleClick >= 2) {
				if ((jiffies - g_DoubleClickJiffies) < (HZ * (SINGLECLICK_INTERVAL
					+ DOUBLECLICK_INTERVAL) / 1000)) {
					/* Double click event */
...
static DEVICE_ATTR_WO(navigation_event);
...
static DEVICE_ATTR_RW(navigation_enable);
