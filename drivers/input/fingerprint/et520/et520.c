...
#define EGIS_GPIO_POWER_ON           "et520_power_active"
#define EGIS_GPIO_POWER_OFF           "et520_power_low"
...
void interrupt_timer_routine(struct timer_list *_data)
{
...
void egis_irq_enable(bool irq_wake, bool enable)
{
	...
	if (irq_wake == 1) {
		if (enable == 1 && g_data->irq_enable_flag == 0) {
		...
		if (enable == 1 && g_data->irq_enable_flag == 0) {
			enable_irq(gpio_irq);
			g_data->irq_enable_flag = 1;
		} else if (enable == 0 && g_data->irq_enable_flag == 1) {
...
int Interrupt_Init(struct etspi_data *etspi, int int_mode, int detect_period, int detect_threshold)
{
	...
	DEBUG_PRINT("FP --  %s request_irq_done = %d gpio_irq = %d  pin = %d\n", __func__,
		request_irq_done, gpio_irq, etspi->irqPin);
...
static DEVICE_ATTR_WO(etspi_enable);
...
static void egis_pinctrl_init(struct etspi_data *etspi)
{

	DEBUG_PRINT("%s  start\n", __func__);
	etspi->power_pinctl = devm_pinctrl_get(&etspi->spi->dev);
	if (IS_ERR(etspi->power_pinctl)) {
		if (PTR_ERR(etspi->power_pinctl) == -EPROBE_DEFER)
			DEBUG_PRINT("pinctrl not ready\n");
		DEBUG_PRINT("Target does not use pinctrl\n");
		etspi->power_pinctl = NULL;
	}

	etspi->power_on = pinctrl_lookup_state(etspi->power_pinctl, EGIS_GPIO_POWER_ON);
	if (IS_ERR(etspi->power_on))
		pr_err("Can't find pinctrl state %s\n", EGIS_GPIO_POWER_ON);

	etspi->power_off = pinctrl_lookup_state(etspi->power_pinctl, EGIS_GPIO_POWER_OFF);
	if (IS_ERR(etspi->power_off))
		pr_err("Can't find pinctrl state %s\n", EGIS_GPIO_POWER_OFF);

	DEBUG_PRINT("%s  successful\n", __func__);

}
static void egistec_power_contrl(struct etspi_data *etspi, int power_onoff)
{
	...
	if (power_onoff) {
		status = pinctrl_select_state(etspi->power_pinctl,
				etspi->power_on);
		if (status)
			DEBUG_PRINT("cannot select et520_power_active\n");
		else
			DEBUG_PRINT("select et520_power_active successful!!!!\n");
	} else {
		status = pinctrl_select_state(etspi->power_pinctl,
				etspi->power_off);
		if (status)
			DEBUG_PRINT("cannot select et520_power_low\n");
		else
			DEBUG_PRINT("select et520_power_low successful!!!!\n");
	}
}
...
static long etspi_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FP_POWER_ONOFF:
		DEBUG_PRINT("fp_ioctl >>> FP_POWER_ONOFF\n");
		if (copy_from_user(&data, (int __user *)arg, sizeof(data))) {
			retval = -EFAULT;
			DEBUG_PRINT("fp_ioctl >>> FP_POWER_ONOFF copy_from_user fail\n");
			goto done;
		}
		egistec_power_contrl(etspi, data.int_mode);
	break;
	...
	case INT_TRIGGER_ABORT:
		...
		fps_interrupt_abort();
		goto done;
	case FP_WAKELOCK_TIMEOUT_ENABLE:
		DEBUG_PRINT("EGISTEC fp_ioctl<<<FP_WAKELOCK_TIMEOUT_ENABLE\n");
		__pm_wakeup_event(&wakeup_source_fp, msecs_to_jiffies(1500));
		goto done;
	case FP_WAKELOCK_TIMEOUT_DISABLE:
...
int etspi_platformInit(struct etspi_data *etspi)
{
	...
	if (etspi != NULL) {
		...
		status = pinctrl_select_state(etspi->power_pinctl, etspi->power_on);
		if (status)
			DEBUG_PRINT("cannot select et520_power_active\n");
		else
			DEBUG_PRINT("select et520_power_active successful!!!!\n");
		...
		*/
		DEBUG_PRINT("et523:the value of etspi->vcc_Pin is %d\n", gpio_get_value(etspi->vcc_Pin));
...
static int etspi_probe(struct platform_device *pdev)
{
	...
	//timer_setup(&fps_ints.timer, interrupt_timer_routine, (unsigned long)&fps_ints);
	...
	request_irq_done = 0;
	push_component_info(FINGERPRINTS, "et520", "egis");
	return status;
