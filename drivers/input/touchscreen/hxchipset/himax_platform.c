...
int drm_tp_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data)
{
...
//	blank = evdata->data;
//	I("drm event:%ld,blank:%ld", event, *blank);
	...
	&& ts->dev != NULL) {
		...
		default:
		I("SUSPEND BLANK(%d) do not need process\n", *blank);
		break;
		}
	}
	...
	&& ts->dev != NULL) {
		...
		default:
		I("RESUME BLANK(%d) do not need process\n", *blank);
		break;
		}
	}
...
int himax_chip_common_probe(struct spi_device *spi)
{
	...
	if (check_dt(dp)) {
		if (!ts_check_default_tp(dp, "qcom,spi-touch-active"))
			ret = -EPROBE_DEFER;
		else
			ret = -ENODEV;
		return ret;
