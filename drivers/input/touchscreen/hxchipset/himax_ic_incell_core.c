		...
		int len, uint8_t *data, uint8_t bank)
{
	...
	w_data = kmalloc_array(data_len, sizeof(uint8_t), GFP_KERNEL);
	if (!w_data)
		return false;
	...
	&& hx_touch_data->hx_rawdata_buf[3] == diag_cmd) {
		...
		for (i = 0; i < RawDataLen_word; i++) {
			...
			if (temp1 < mul_num) { /*mutual*/
				mutual_data[index + i] =
					((int8_t)
					hx_touch_data->hx_rawdata_buf[i*2+4+1])
					* 256+
...
static int himax_mcu_chip_self_test(struct seq_file *s, void *v)
{
	...
	if (test_result_id == pfw_op->data_selftest_pass[0]) {
		...
		seq_puts(s, "0 error(s). All test passed.\n");
		pf_value = 0x0;
	} else {
...
static void himax_mcu_read_FW_ver(void)
{
	...
	count = snprintf(Ctp_name, sizeof(Ctp_name), "TM,HX83112,FW:0x%x\n", ic_data->vendor_touch_cfg_ver);
	I("TOUCH_VER : %X\n", ic_data->vendor_touch_cfg_ver);
...
static int hx_read_guest_info(void)
{
	...
	int i = 0;
	unsigned int custom_info = 0;
	int checksum = 0;
	struct hx_guest_info *gst_info = g_guest_info_data;
	...
	for (custom_info = 0;
	custom_info < HX_GUEST_INFO_SIZE;
	custom_info++) {
		checksum = himax_guest_info_read(panel_info_addr
			+ custom_info
			* flash_page_len,
			&flash_tmp_buffer[custom_info * flash_page_len]);
		if (checksum != 0) {
			E("%s:Checksum Fail! g_checksum_str len=%d\n", __func__,
				(int)strlen(g_checksum_str));
			memcpy(&gst_info->g_guest_str_in_format[custom_info][0],
				g_checksum_str, (int)strlen(g_checksum_str));
			memcpy(&gst_info->g_guest_str[custom_info][0],
				g_checksum_str, (int)strlen(g_checksum_str));
			continue;
		}

		info_len = flash_tmp_buffer[custom_info * flash_page_len]
			+ (flash_tmp_buffer[custom_info
			* flash_page_len + 1] << 8)
			+ (flash_tmp_buffer[custom_info
			* flash_page_len + 2] << 16)
			+ (flash_tmp_buffer[custom_info
		...
		I("Now size_buff[0]=0x%02X,[1]=0x%02X,[2]=0x%02X,[3]=0x%02X\n",
			flash_tmp_buffer[custom_info*flash_page_len],
			flash_tmp_buffer[custom_info*flash_page_len + 1],
			flash_tmp_buffer[custom_info*flash_page_len + 2],
			flash_tmp_buffer[custom_info*flash_page_len + 3]);

		I("Now total length=%d\n", info_len);

		gst_info->g_guest_data_len[custom_info] =
			info_len;

		I("Now custom_info_id [0]=%d,[1]=%d,[2]=%d,[3]=%d\n",
			flash_tmp_buffer[custom_info*flash_page_len + 4],
			flash_tmp_buffer[custom_info*flash_page_len + 5],
			flash_tmp_buffer[custom_info*flash_page_len + 6],
			flash_tmp_buffer[custom_info*flash_page_len + 7]);

		gst_info->g_guest_data_type[custom_info_temp] =
			flash_tmp_buffer[custom_info * flash_page_len
			+ 7];
		...
		for (i = 0; i < info_len; i++)
			temp_str[i] = flash_tmp_buffer[custom_info
				* flash_page_len
				+ HX_GUEST_INFO_LEN_SIZE
				+ HX_GUEST_INFO_ID_SIZE
				+ i];

		I("ggst_info->g_guest_str_in_format[%d]size=%d\n",
			custom_info, info_len);
		memcpy(&gst_info->g_guest_str_in_format[custom_info][0],
			temp_str, info_len);
		/*}*/
		...
		I("gst_info->g_guest_str[%d] size = %d\n",
				custom_info, 128);
		memcpy(&gst_info->g_guest_str[custom_info][0],
				temp_str, 128);
		/*if(custom_info == 0)
		 ...
		 *		if(i % 16 == 0 && i > 0)
		 *			I("\n");
		 *		I("gst_info->g_guest_str[%d][%d]
					= 0x%02X", custom_info, i,
					gst_info->g_guest_str[
					custom_info][i]);
		 *	}
		 *}
		 */
