...
struct proc_dir_entry *himax_touchpanel_proc_dir;
EXPORT_SYMBOL(himax_touchpanel_proc_dir);


struct proc_dir_entry *oneplus_touch_proc_dir;
EXPORT_SYMBOL(oneplus_touch_proc_dir);
...
#define ONEPLUS_PROC_TOUCH_FOLDER "touchpanel"
/*ts_work about start*/
...
#define ONEPLUS_PROC_GESTURE_ENABLE_FILE	"gesture_enable"
struct proc_dir_entry *oneplus_proc_gesture_enable_file;

#define ONEPLUS_PROC_COOR_FILE	"coordinate"
struct proc_dir_entry *oneplus_proc_coor_file;

#define ONEPLUS_PROC_GESTURE_SWITCH_FILE	"gesture_switch"
struct proc_dir_entry *oneplus_proc_gesture_switch_file;


#define HIMAX_PROC_SELF_TEST_FILE	"self_test"
struct proc_dir_entry *himax_proc_self_test_file;

#define ONEPLUS_PROC_BASELINE_TEST_FILE	"ctp_openshort_test"
struct proc_dir_entry *oneplus_proc_baseline_test_file;
...
static int32_t himax_coor_show(struct seq_file *m, void *v)
{
	...
	I("%s enter.\n", __func__);
		...
		i = snprintf(temp_buf, sizeof(temp_buf), "%d,%d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%d",
			hx_gesture_coor[0],
...
const static struct file_operations himax_proc_coor_ops = {
	.owner = THIS_MODULE,
...
static uint8_t DouTap_enable;				 // double tap
static uint8_t UpVee_enable;				 // V
static uint8_t LeftVee_enable;			// >
static uint8_t RightVee_enable;			 // <
static uint8_t Circle_enable;				 // O
static uint8_t DouSwip_enable;			// ||
static uint8_t Mgestrue_enable;			 // M
static uint8_t Wgestrue_enable;			 // W
static uint8_t Sgestrue_enable;			 // S
static uint8_t SingleTap_enable;			 // single tap

static ssize_t himax_gesture_enable_read(struct file *file, char *buf,
		size_t len, loff_t *pos)
{
	struct himax_ts_data *ts = private_ts;
	int i = 0;
...
static ssize_t himax_gesture_enable_write(struct file *file, const char *buff,
		size_t count, loff_t *ppos)
{
	...
	I("%s write argc1[0x%x],argc2[0x%x]\n", __func__, buf[0], buf[1]);
	...
	if (UpVee_enable || DouSwip_enable || LeftVee_enable || RightVee_enable
			...
			|| Wgestrue_enable || SingleTap_enable) {
		value = 1;
	} else {
		value = 0;
	}

	gesture_mode_value = value;//for lcd control power

	if (ts->SMWP_enable != value) {
		ts->SMWP_enable = value;
		g_core_fp.fp_set_SMWP_enable(ts->SMWP_enable, ts->suspended);
	}
...
static const struct file_operations himax_proc_gesture_enable_ops = {
	.owner = THIS_MODULE,
	.read = himax_gesture_enable_read,
	.write = himax_gesture_enable_write,
};


int himax_oneplus_proc_touchpanel_init(void)
{

	oneplus_touch_proc_dir = proc_mkdir(ONEPLUS_PROC_TOUCH_FOLDER, NULL);
	if (oneplus_touch_proc_dir == NULL) {
		E(" %s: oneplus_touch_proc_dir file create failed!\n", __func__);
		return -ENOMEM;
	}

#ifdef HX_SMART_WAKEUP

	oneplus_proc_gesture_enable_file = proc_create(ONEPLUS_PROC_GESTURE_ENABLE_FILE, 0666,
							oneplus_touch_proc_dir, &himax_proc_gesture_enable_ops);
	if (oneplus_proc_gesture_enable_file == NULL) {
		E(" %s: proc gesture enable file create failed!\n", __func__);
		return -ENOMEM;
	}
#endif
	...
	oneplus_proc_coor_file = proc_create(ONEPLUS_PROC_COOR_FILE, 0666,
							oneplus_touch_proc_dir, &himax_proc_coor_ops);
	if (oneplus_proc_coor_file == NULL) {
		E(" %s: proc coorinate file create failed!\n", __func__);
		goto coorinate_file_creat_fail;
	}
#endif

	oneplus_proc_gesture_switch_file = proc_create(ONEPLUS_PROC_GESTURE_SWITCH_FILE, 0666,
							oneplus_touch_proc_dir, &himax_proc_psensor_ops);
	if (oneplus_proc_gesture_switch_file == NULL) {
		E(" %s: proc gesture switch file create failed!\n", __func__);
		goto gesture_switch_file_creat_fail;
	}

	return 0;
gesture_switch_file_creat_fail:
#ifdef HX_GESTURE_TRACK
	remove_proc_entry(ONEPLUS_PROC_COOR_FILE, oneplus_touch_proc_dir);
#endif
coorinate_file_creat_fail:
#ifdef HX_SMART_WAKEUP
	remove_proc_entry(ONEPLUS_PROC_GESTURE_ENABLE_FILE, oneplus_touch_proc_dir);
#endif
...
*/
	if (himax_oneplus_proc_touchpanel_init() < 0)
		return -ENOMEM;
	...
	oneplus_proc_baseline_test_file = proc_create(ONEPLUS_PROC_BASELINE_TEST_FILE, 0666,
		oneplus_touch_proc_dir, &himax_proc_self_test_ops);
	if (oneplus_proc_baseline_test_file == NULL) {
...
void himax_oneplus_proc_touchpanel_deinit(void)
{
#ifdef HX_SMART_WAKEUP
	remove_proc_entry(ONEPLUS_PROC_GESTURE_ENABLE_FILE, oneplus_touch_proc_dir);
#endif
	...
	remove_proc_entry(ONEPLUS_PROC_COOR_FILE, oneplus_touch_proc_dir);
#endif
	remove_proc_entry(ONEPLUS_PROC_GESTURE_SWITCH_FILE, oneplus_touch_proc_dir);
}
...
static void gest_pt_log_coordinate(int rx, int tx)
{
	...
	gest_pt_x[gest_pt_cnt] =
			(rx == 255 ? 0 : (rx * (ic_data->HX_X_RES) / 255));
	gest_pt_y[gest_pt_cnt] =
			(tx == 255 ? 0 : (tx * (ic_data->HX_Y_RES) / 255));
...
static int himax_wake_event_parse(struct himax_ts_data *ts, int ts_status)
{
	...
	buf = kcalloc(hx_touch_data->event_size * sizeof(uint8_t), sizeof(uint8_t), GFP_KERNEL);
	if (buf == NULL)
		return -ENOMEM;
	...
	if (buf[GEST_PTLG_ID_LEN] == GEST_PTLG_HDR_ID1 &&
		...
		while (i < gest_len) {
			if (i != 6) {
				fwrx = buf[GEST_PTLG_ID_LEN + 4 + i * 2];
				fwtx = buf[GEST_PTLG_ID_LEN + 4 + i * 2 + 1];
				gest_pt_log_coordinate(fwrx, fwtx);
				hx_gesture_coor[i * 2 + 1] =
				(gest_pt_x[gest_pt_cnt] == 255 ? 0 : gest_pt_x[gest_pt_cnt]);
				hx_gesture_coor[i * 2 + 2] =
				(gest_pt_y[gest_pt_cnt] == 255 ? 0 : gest_pt_y[gest_pt_cnt]);
				I("hx_gest_coor[%d]=%d\n", i * 2 + 1, hx_gesture_coor[i * 2 + 1]);
				I("hx_gest_coor[%d]=%d\n", i * 2 + 2, hx_gesture_coor[i * 2 + 2]);
				gest_pt_cnt++;
			} else {
				hx_gesture_coor[i * 2 + 1] =
				(buf[GEST_PTLG_ID_LEN + 4 + i * 2] == 255 ? 0 : buf[GEST_PTLG_ID_LEN + 4 + i * 2]);
				I("hx_gest_coor[%d]=%d\n", i * 2 + 1, hx_gesture_coor[i * 2 + 1]);
			}
			i++;
		}
...
static void himax_fb_register(struct work_struct *work)
{
	...
	hx_msm_drm_register_client =
		(void *)kallsyms_lookup_name("drm_panel_notifier_register");
	if (hx_msm_drm_register_client != NULL)
		ts->fb_notif.notifier_call = drm_tp_notifier_callback;
	else
		E("hx_msm_drm_register_client is NULL\n");
	if (tp_active_panel &&
		drm_panel_notifier_register(tp_active_panel,
			&ts->fb_notif) < 0)
		I("register notifier failed!\n");
#endif
}
#endif
...
void himax_chip_common_deinit(void)
{
	...
	himax_oneplus_proc_touchpanel_deinit();
...
#else
	if (tp_active_panel)
		drm_panel_notifier_unregister(tp_active_panel, &ts->fb_notif);
#endif
	cancel_delayed_work_sync(&ts->work_att);
	destroy_workqueue(ts->himax_att_wq);
#endif
